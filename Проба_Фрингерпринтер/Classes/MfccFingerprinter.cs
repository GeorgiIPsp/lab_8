...
namespace Проба_Фрингерпринтер.Classes
{
    public class MfccFingerprinter
    {
        ...
        public void PlotSingleAudioWaveform(string audioPath, string outputPath = "audio_waveform.png")
        {
            try
            {
                // Загрузка и предобработка аудио
        ...
        public float[] LoadAndPreprocessAudio(string audioPath)
        {
        ...
        public void NormalizeSamples(float[] samples)
        {
        ...
        /// <returns></returns>
        public float[] TrimSilence(float[] audioData, float threshold)
        {
            int start = 0;
        ...
        public List<float[]> GetFrames(float[] audioData, int frameSize, int frameStep)
        {
